...
export default function PlayNotes() {
  ...
  const renderImages = () => {
    const validChars = name.replace(/[^a-z]/g, ''); // Hanya huruf a-z
  ...
  return (
    <div className="flex fixed bg-[#CC1419] h-full w-full overflow-auto flex-col items-center justify-center">
      ...
      <div className={`relative w-full ${udahIsiNama ? 'hidden' : ''}`}>
        <h1 className={`text-center text-3xl lg:text-6xl font-medium mt-1 lg:mt-10 ${kanit.className}`}>
          START WITH <br></br>
          ENTERING YOUR NAME
        </h1>

        <div className="mx-5" style={{ marginTop: '20px', display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
          {renderImages()}
        </div>
