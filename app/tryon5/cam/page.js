...
export default function Register() {
    ...
    useEffect(() => {
        ...
        if (typeof localStorage !== 'undefined') {
            const item1 = localStorage.getItem('clothesImage')
            setClothes(item1)
        }
    }, [clothes])
    ...
    const generateImage = async () => {
        ...
        try {
            ...
            const result = await fal.subscribe('fal-ai/nano-banana-pro/edit', {
                input: {
                    prompt:`add the clothes in first image to person in second image`,
                    image_urls
                },
                pollInterval: 5000,
                logs: true,
    ...
    return (
        <main className="flex min-h-screen bg-purina-ps flex-col items-center justify-top pt-2 pb-5 px-5 lg:pt-12 lg:px-20">
            <TopLogo></TopLogo>
