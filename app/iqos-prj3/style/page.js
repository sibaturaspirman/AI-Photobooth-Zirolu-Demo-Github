...
export default function Register() {
    ...
    const generateAI = () => {
        ...
        if(character == 'cowok'){
            if (typeof localStorage !== 'undefined') {
                ...
                localStorage.setItem("urlFix", "https://ai.zirolu.id/iqos/prj/MALE-"+urutanFIX+"-MOD.jpg")
            }
        }else{
            if(!hijab){
                if (typeof localStorage !== 'undefined') {
                    localStorage.setItem("formasiFix", 'FEMALE')
                    localStorage.setItem("urlFix", "https://ai.zirolu.id/iqos/prj/FEMALE-"+urutanFIX+"-MOD.jpg")
                }
            }else{
                if (typeof localStorage !== 'undefined') {
    ...
    return (
        <main className="flex fixed h-full w-full bg-iqos-prj overflow-auto flex-col items-center justify-center pt-2 pb-5 px-5 lg:pt-12 lg:px-20" onContextMenu={(e)=> e.preventDefault()}>
            ...
            <Image src='/iqos/prj-select.png' width={913} height={104} alt='Zirolu' className='w-full' priority />
            </div>
            {/* PILIH STYLE */}
            <div className={`relative w-[90%] mx-auto mt-10`}>
                <div className='relative mt-0 w-full bg-black p-20 rounded-2xl'>
                    <div className="relative w-[30%] lg:w-[90%] mx-auto mb-10">
                ...
                {/* {character && */}
                    <div className={`relative w-full flex justify-center items-center mt-2 lg:mt-[9rem] z-20 ${character ? `` : 'opacity-0 pointer-events-none'}`}>
                        <button className="relative mx-auto w-[100%] flex justify-center items-center" onClick={generateAI}>
                            <Image src='/iqos/prj-btn-continue.png' width={850} height={258} alt='Zirolu' className='w-full' priority />
                        </button>
                    </div>
                {/* } */}
            </div>
