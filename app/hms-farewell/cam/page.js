...
export default function Cam() {
    ...
    }, [styleFix, styleFix2, styleFix3, formasiFix])

    const generateAI = () => {
        setNumProses1(true)
        generateLeftRigtBase()
    }
    ...
    const generateLeftRigtBase = async () => {
        setNumProses(2)
        toDataURL(styleFix)
        .then(dataUrl => {
            if (typeof localStorage !== 'undefined') {
                localStorage.setItem("resulAIBase64Left", dataUrl)
            }
            setTimeout(() => {
                setNumProses(3)
                toDataURL(styleFix3)
                .then(dataUrl2 => {
                    if (typeof localStorage !== 'undefined') {
                        localStorage.setItem("resulAIBase64Right", dataUrl2)
                    }
                    setTimeout(() => {
                        generateImageSwap()
                    }, 500);
    ...
    const generateImageSwap = async () => {
        ...
        .then(dataUrl => {
            ...
            setTimeout(() => {
                router.push('/hms-farewell/result');
            }, 500);
        })
        } catch (error) {
            setError(error);
    ...
    return (
        <main className="flex fixed h-full w-full bg-hms overflow-auto flex-col items-center pt-2 pb-5 px-5 lg:pt-12 lg:px-20">
            <div className="relative w-[30%] mx-auto mt-44">
            <Image src='/hms/title-takephoto.png' width={264} height={64} alt='Zirolu' className='w-full' priority />
            </div>
            {/* LOADING */}
            {numProses1 && 
            ...
            {!enabled && 
                <div className="relative w-full flex justify-center items-center">
                    <button className="relative mx-auto flex  w-[100%] justify-center items-center" onClick={captureVideo}>
                        <Image src='/hms/btn-capture.png' width={640} height={88} alt='Zirolu' className='w-full' priority />
                    </button>
                </div>
            }
            ...
            <div className={`relative w-full ${!enabled ? 'hidden' : ''}`}>
                <div className="relative w-[100%] mx-auto flex justify-center items-center flex-col mt-0">
                    <button className="w-full relative mx-auto flex justify-center items-center" onClick={generateAI}>
                        <Image src='/hms/btn-generate.png' width={640} height={88} alt='Zirolu' className='w-full' priority />
                    </button>
