...
export default function Register() {
    ...
    const generateAI = () => {
        ...
        if(character == 'cowok'){
            ...
            if(styleFemale == 'normal'){
                ...
                let randomGambar = getRandomInt(1,2);
                urlGambar = 'https://ai.zirolu.id/pln/style/h-'+randomGambar+'.jpg';
    ...
    return (
        <main className="flex fixed h-full w-full bg-pln overflow-auto flex-col items-center justify-center pt-2 pb-5 px-5 lg:pt-12 lg:px-20" onContextMenu={(e)=> e.preventDefault()}>
            ...
            <Image src='/pln/identify.png' width={685} height={51} alt='Zirolu' className='w-full' priority />
            </div>
            {/* PILIH STYLE */}
                ...
                {/* {character && */}
                    <div className={`relative w-full flex justify-center items-center mt-[22rem] z-20 ${character ? `` : 'opacity-0 pointer-events-none'}`}>
                        <button className="relative mx-auto w-[100%] flex justify-center items-center" onClick={generateAI}>
                            <Image src='/pln/btn-continue.png' width={644} height={144} alt='Zirolu' className='w-full' priority />
                        </button>
                    </div>
                {/* } */}
            </div>
